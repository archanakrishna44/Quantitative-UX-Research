...
**Causal overclaim.** Two layers enforce this. First, the S2 framing rules require the research question to be framed correlationally unless a causal identification strategy is specified. Second, `validate_results_md.py` bans a list of causal language terms (`causes`, `effect of`, `led to`, `resulted in`, etc.) when the `causal_identification_strategy` in the S6 plan is `none`. The ban is enforced at the text level before results are presented to the user.
